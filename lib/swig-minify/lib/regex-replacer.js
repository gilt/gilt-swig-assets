...
Position.prototype.forward = function(content) {
  var lines = content.split(lineMatcher);
...
Position.prototype.add = function(node, content, autoForward) {
  ...
  var pos = autoForward ? this : new Position(this.file, this.line, this.column);

  var match;
  var sub;

  var lastIndex = lineMatcher.lastIndex = 0;
...
var Replacer = module.exports = function Replacer(regexp, replace, regexpName) {
  // Handle params
...
Replacer.prototype.createReplaceNode = function(pos, match) {
  ...
  var varMatch;
  var replacePos = new Position(this.$regexpName);
  const $replace = typeof this.$replace === 'function'
      ? this.$replace()
      : this.$replace;

  var lastIndex = varMatcher.lastIndex = 0;
  while ((varMatch = varMatcher.exec($replace))) {
    var parValue = match[varMatch[2] || varMatch[1]];
...
Replacer.prototype.replace = function(content, file) {
  ...
  var lastIndex = this.$regexp.lastIndex = 0;
  ...
  var res = node.toStringWithSourceMap({file: file});
